...
class HistoryStore:
    ...
    def all(self) -> list[PredictionResponse]:
        return [PredictionResponse.model_validate(item) for item in self._read()]

    def get(self, prediction_id: str) -> PredictionResponse | None:
        for item in self.all():
            if item.id == prediction_id:
                return item
        return None

    def add(self, prediction: dict) -> PredictionResponse:
        items = self._read()
        record = {
            "id": str(uuid4()),
    ...
    def delete(self, prediction_id: str) -> bool:
        items = self._read()
        next_items = [item for item in items if item.get("id") != prediction_id]
